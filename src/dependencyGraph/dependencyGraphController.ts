        ...
        private constructGraph(csvData: string) {
            ...
            rows.slice(1).forEach((row) => {
                ...
                for (var semesterIndex = 4; semesterIndex < row.length; semesterIndex++) {
                    if (row[semesterIndex] && row[semesterIndex].toUpperCase() === row[semesterIndex]) {
                        var recitations = row[semesterIndex].split("");
                        recitations.forEach((recitation) => safeGet(header[semesterIndex], recitation).cas.push(andrewId));
                    }
                }
            });
            ...
            (<any>graph).graph().nodesep = 200;
            (<any>graph).graph().ranksep = 150;
            ...
            Object.keys(semesterRecitationInfo).forEach((semester) => {
                Object.keys(semesterRecitationInfo[semester]).forEach((recitation) => {
                    var recitations;
                    if (recitation.length == 1) {
                        recitations = [recitation];
                    } else if (recitation.length == 2) {
                    ...
                    recitationInfo.cas.forEach((parent) => {
                        recitationInfo.students.forEach((child) => {
                            (<any>graph).setEdge(parent, child, {
